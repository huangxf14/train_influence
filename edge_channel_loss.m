    ...
    while cnt<slot_tot
        ...
        for device_cnt=1:device_num
            for device_sample_cnt=1:best_sample_num
                ...
                inputVector = device_data(device_cnt,:,device_best_sample(device_cnt,device_sample_cnt));
        ...
        if true
            device_error_exp = device_sample_importance(:,1).^2./ceil(10./device_v);
...
%       data = device_data(last_device,:,device_index(last_device)+1);
        data = device_data(last_device,:,device_best_sample(last_device,1));
    
        targetVector = device_label(last_device,:,device_best_sample(last_device,1))';
        policy = 4;
        while policy>1
            tempdata = reshape(imresize(imresize(reshape(data,[28,28]),[7*policy,7*policy]),[28,28]),[28*28,1])';
            temp_importance = norm(activationFunction(outputWeights*activationFunction(hiddenWeights*tempdata')) - targetVector, 2);
            tempdata = reshape(imresize(imresize(reshape(data,[28,28]),[7*policy-7,7*policy-7]),[28,28]),[28*28,1])';
            temp_importance2 = norm(activationFunction(outputWeights*activationFunction(hiddenWeights*tempdata')) - targetVector, 2);
            if (temp_importance - temp_importance2 > temp_importance * 0.05)
                break;
            end
%             if (temp_importance2 > theta * test_error)||(abs(temp_importance-temp_importance2)>temp_importance2*theta_change)
            if (abs(temp_importance-temp_importance2)>temp_importance2*theta_change)
                break;
            end
            policy = policy - 1;     
        end
...
%        device_error(last_device) = device_error(last_device) * 0.5 + 0.5 * sample_error/error_tot;
    ...
    plot(accuracy_array);
    save('device_i_v_2_14_n10_compress','accuracy_array','device_index');
